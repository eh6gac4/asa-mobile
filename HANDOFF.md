...
> 補足(2026-08-05): 一時的にコード一式(wrangler.toml / package.json / src/index.js /
> README.md)がこのディレクトリから欠落していたため、本メモのアーキテクチャ節を仕様として
> 再作成した。ロジックはこのメモの記述と一致させてある。git管理も今回から開始。
> Cloudflareアカウント認証とGemini APIキーが要る手順(下記1〜6)は未実施のまま。
